...
`build.sh` prefers the `dasm` Docker image and falls back to a local `dasm` binary in `PATH`. It assembles with DASM output format `-f1` (PRG), so the file carries the load address `$0401` in its first two bytes. It then refreshes `disk/work.d64` so the on-disk copy of the program stays current.

Output:

- `build/commander.prg` - the program, load address `$0401`.
...
```
disk/build-work-d64.sh
```

This formats `disk/work.d64` with `c1541` (ships with VICE) and writes `commander.prg` plus a small set of mixed-type sample files. `build.sh` calls it automatically after a successful assemble.
...
```
./run.sh                 # autostart disk/work.d64 on drive 8
...
- Write it (and optional sample files) to a real disk with a compatible drive, or transfer `disk/work.d64` to physical media using a suitable hardware tool.
